...
SUGGESTED_QUESTIONS = (
    ("Resumo do período", "Como estamos no período?"),
    ("Prioridades comerciais", "Quais são as prioridades comerciais?"),
    ("Drivers da variação", "Por que o faturamento variou no período?"),
    ("Vendedores em risco", "Quais vendedores estão em risco?"),
    ("Concentração de clientes", "Qual a concentração dos clientes?"),
    ("Margem negativa", "Quais produtos têm margem negativa?"),
    ("Clientes sem comprar", "Quais clientes estão sem comprar há 90 dias?"),
    ("Projeção de fechamento", "Qual a projeção de fechamento?"),
    ("Oportunidades", "Quais são as oportunidades de venda?"),
)
...
def answer(question,df,history_df=None,start_date=None,end_date=None):
    ...
    if df is None or df.empty or "Data" not in df:
        return "Não há registros no recorte atual. Amplie o período ou ajuste os filtros para analisar este tema."
    ...
    if any(k in q for k in ["onde agir","prioridades","prioridade comercial","command center","plano de ação","plano de acao"]):
        ...
        growth = cc["revenue_delta"] / cc["previous"]["revenue"] if cc["previous"]["revenue"] else None
    ...
    if any(k in q for k in ["vendedores em risco","performance dos vendedores","ranking de vendedores"]):
        ...
        return "### Performance dos vendedores\n" + (lines or "Não há vendedores no recorte atual.")
    if any(k in q for k in ["resumo executivo","diagnóstico geral","diagnostico geral","como estamos"]):
        ...
        rev=cm["revenue"]/pm["revenue"]-1 if pm["revenue"] else None; mpp=(cm["margin_pct"]-pm["margin_pct"])*100
        ...
        variation = pct(rev) if rev is not None else "sem base comparável"
        reading = (
            "crescimento com melhora de qualidade" if rev is not None and rev > 0 and mpp >= 0
            else "crescimento com compressão de margem" if rev is not None and rev > 0
            else "retração que exige recuperação dos principais detratores" if rev is not None
            else "primeiro período disponível para comparação"
        )
        return f"### Resumo executivo\n- Faturamento: **{money(cm['revenue'])}** (**{variation}** vs período anterior).\n- Margem: **{money(cm['margin'])}**, equivalente a **{pct(cm['margin_pct'])}** (**{mpp:+.1f} p.p.**).\n- Maior contribuição positiva: {best_txt}.\n- Maior pressão: {worst_txt}.\n- Top 5 clientes concentram **{pct(cmp['concentration_top5'])}** da receita.\n\n**Leitura:** {reading}."
    if any(k in q for k in ["por que","porque","driver","explica","causa","caiu","subiu","avançou","recuou"]):
        ...
        delta=cm["revenue"]-pm["revenue"]; growth=delta/pm["revenue"] if pm["revenue"] else None
        pos=cmp["comparison"].nlargest(5,"delta_faturamento"); neg=cmp["comparison"].nsmallest(5,"delta_faturamento")
        variation = pct(growth) if growth is not None else "sem base comparável"
        positive_lines = _top_lines(pos, dim, "delta_faturamento", 5) or "Sem contribuição positiva no período."
        negative_lines = _top_lines(neg, dim, "delta_faturamento", 5) or "Sem pressão negativa no período."
        return f"### Diagnóstico por {dim}\nO faturamento variou **{money(delta)} ({variation})** contra o período anterior de mesma duração.\n\n**Principais contribuições positivas**\n{positive_lines}\n\n**Principais pressões**\n{negative_lines}\n\n**Próxima análise:** valide volume, preço/kg e margem dos três maiores detratores antes de definir a ação comercial."
    ...
    if any(k in q for k in ["risco","atenção","atencao","alerta"]):
        c=client_classification(history_df, end_date); risk=c[(c.abc=='A')&(c.dias_sem_compra>=90)]; neg=df[df.Margem<0]; below=df[(df['Benchmark Grupo']>0)&(df['Preço Real Kg']<df['Benchmark Grupo']*.95)]
    ...
    if "margem negativa" in q:
        t=group_metrics(df,"Produto"); t=t[t["margem"]<0].sort_values("margem").head(15); return "\n".join([f"{i+1}. **{r.Produto}**: {money(r.margem)} | {pct(r.margem_pct)}" for i,r in t.iterrows()]) or "Não há produtos com margem negativa no recorte atual."
    m=re.search(r"(\d+)\s*dias",q)
    if ("sem comprar" in q or "inativ" in q) and m:
        days=int(m.group(1)); c=client_classification(history_df, end_date); c=c[c["dias_sem_compra"]>=days].sort_values("faturamento",ascending=False).head(15); return "\n".join([f"{i+1}. **{r.Cliente}**: {r.dias_sem_compra} dias | histórico {money(r.faturamento)}" for i,r in c.iterrows()]) or f"Não há clientes com {days} dias ou mais sem comprar no histórico disponível."
    ...
    if "o que vender" in q or "oportunidade" in q:
        o=build_opportunities(df).head(15); return "\n".join([f"{i+1}. **{r.Cliente}** → {r.Produto} | {r.acao} | score {r.score:.0f}" for i,r in o.iterrows()]) or "Não há oportunidades qualificadas no recorte atual."
